...
// Parse une valeur de stock : nombre entier 0–999 (défaut 0).
...
// Parse l'unité : nombre de pièces par paquet (entier 1–99) → libellé « N pièces / paquet ».
function parseUnit(raw, category) {
  const n = Math.min(99, Math.max(0, parseInt(String(raw ?? ''), 10) || 0))
  if (n > 0) return `${n} pièces / paquet`
  return category === 'surgele' ? 'Paquet à congeler' : '10 pièces / paquet'
}
...
function normalizeFields(fields) {
  ...
  const base = {
    ...
    price,
    unit: parseUnit(fields.unit, category),
